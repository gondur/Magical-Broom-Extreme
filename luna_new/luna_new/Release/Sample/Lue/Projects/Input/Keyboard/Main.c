...
int WINAPI WinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow )
{
	...
	while ( Lue_Run( LFALSE ) )
	{
		...
		// キーボードの入力に応じて絵の移動
		//  キーの定義に関してはリファレンスを参照して下さい。
		if ( LueInput_GetKeyData( LUE_VK_UP    ) ) Py -= 2;	// 上
		if ( LueInput_GetKeyData( LUE_VK_DOWN  ) ) Py += 2;	// 下
		if ( LueInput_GetKeyData( LUE_VK_LEFT  ) ) Px -= 2;	// 左
		if ( LueInput_GetKeyData( LUE_VK_RIGHT ) ) Px += 2;	// 右
		...
		Src.h = TEX_H;
		// 読み込んだテクスチャを画面に描画
		LueRender_DrawSingleTexture( hTex, Dst, Src, LUE_RGB(255,255,255) );
