...
int WINAPI WinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow )
{
	...
	if ( pCore->Initialize( L"Selene Sample Program", FRAME_RATE_60 ) )
	{
		...
		// バンプマップ用テクスチャ
		//-----------------------------------------------------------------
		// モデルに貼るバンプマッピング用テクスチャ
		//   グレースケールの画像を高さ情報として保存してある
		//   テクスチャをバンプマップとして使用します。
		pTexNormal = pRender->CreateTextureFromFileForNormalMap( L"height.bmp", 3.0f );
		...
		for ( i = 0; i < MeshCnt; i++ )
		{
			...
			// パララックス用メッシュに変換
			pMesh->ConvertParallaxMesh( pTexNormal, 0.03f );
