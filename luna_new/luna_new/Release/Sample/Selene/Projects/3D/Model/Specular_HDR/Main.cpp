...
int WINAPI WinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow )
{
	...
	if ( pCore->Initialize( L"Selene Sample Program", FRAME_RATE_60 ) )
	{
		...
		pSceneMgr = pRender->CreateSceneManager( SCENE_MODE_MANAGED, SCENE_MODE_OPTION_HDR );
		...
		// メッシュ数取得
		MeshCnt = pModel->GetMeshCount();
		for ( i = 0; i < MeshCnt; i++ )
		{
			pModel->GetMesh( i, pMesh );
			// 鏡面反射用メッシュに変換
			pMesh->ConvertReflexMesh();
			// メッシュに負荷情報
			MtrlCnt = pMesh->GetMaterialCount();
			for ( j = 0; j < MtrlCnt; j++ )
			{
				// マテリアルにスペキュラの色を設定
				pMesh->SetSpecularColor( j, CColor(64,255,255) );
				pMesh->SetSpecularRefractive( j, 2.0f );
				pMesh->SetSpecularRoughly( j, 0.1f );
			}
			pMesh->Release();
		}
