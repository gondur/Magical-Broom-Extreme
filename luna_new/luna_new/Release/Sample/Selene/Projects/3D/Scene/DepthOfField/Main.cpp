...
int WINAPI WinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow )
{
	...
	if ( pCore->Initialize( L"Selene Sample Program", FRAME_RATE_60 ) )
	{
		...
		pSceneMgr = pRender->CreateSceneManager( SCENE_MODE_1PASS_DOF );
		...
		pCamera->UpdateProjection( 10.0f, 150.0f, DEG_TO_ANGLE(45), SCREEN_WIDTH, SCREEN_HEIGHT );
		...
		while ( pCore->Run() )
		{
			...
			//   CStyleクラスで変換をかけたものを
			//   カメラに渡します。
			CVector3D vEye( 0.0f, 30.0f, -80.0f );
			...
			// 被写界深度のフォーカスを設定
			//   どの位置にピントを合わせているかを設定します。
			//   基本的に、カメラのニアクリップからファークリップの間になります。
			Float fForcus = (Math::Sin( pCore->GetSyncCount() * 128 ) * 50.0f) + 70.0f;
			pSceneMgr->SetDepthOfFieldForcus( fForcus, 3.0f );
